...
const { modelValue, carrierSelected } = defineProps({
  modelValue: {
    type: Boolean,
    default: false,
  },
...
const emit = defineEmits(['update:modelValue', 'onSelectPointRelay']);
...
const sideRelayPointsVisible = computed({
  get: () => modelValue,
...
<template>
  ...
  >
    ...
    <div class="flex flex-col h-full">
      ...
      <div class="-mx-5 flex-1 border-t border-black -mb-5" v-loading="loading">
        <BaseTabs v-if="relayPointsSerched.length > 0" default-tab="tab-list">
            ...
            <div class="relayPoints-list overflow-auto">
              <div v-for="rp in relayPointsSerched" :key="rp.Id">
                ...
                >
                  ...
                  />
                  <span v-if="rp?.Name" class="font-normal mb-1">{{ rp?.Name }}</span>
                  <span v-if="rp?.Address1">{{ rp?.Address1 }}</span>
                  <span v-if="rp?.Address2">{{ rp?.Address2 }}</span>
                  <span v-if="rp?.Address3">{{ rp?.Address3 }}</span>
                  <span v-if="rp?.Postcode">
                    {{ rp?.Postcode }}
                    <span v-if="rp?.City">{{ rp?.City }}</span>
                  </span>
                  <div class="mt-3 flex gap-5">
                    <span v-if="rp?.Location?.DistanceFromAddress" class="font-normal">
                  ...
                  <transition name="slide">
                    <div
                      ...
                      v-if="rp?.OpeningHours && rpSchedulesOpen === rp.Id"
                    >
                      <div
                        v-for="(oh, index) in rp?.OpeningHours"
                        :key="oh.Day"
                        class="flex text-xs border-black"
                        :class="{
                          'border-b': index < Object.keys(rp?.OpeningHours || {}).length - 1,
                        }"
                      >
                        <span class="text-xs min-w-32 font-light border-r border-black py-1 px-3">
                          {{ oh.Day }} :
                        </span>
          ...
          <BaseTabsItem name="tab-map" :label="$t('label.relayPointMap')">
            <div class="relayPoints-list">
              <BaseGoogleMap
                ...
                :zoom="14"
                :markers="markers"
                :selectedMarkerId="selectedMarkerId || relayPointSelectedID || 0"
